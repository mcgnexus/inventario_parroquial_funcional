...
import { LoginSchema, createErrorResponse, createSuccessResponse } from '@/schemas'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validationResult.success) {
      ...
      console.log('[API Login] Error de validación:', errors.join(', '))
      return NextResponse.json(createErrorResponse(errors.join(', ')), { status: 400 })
    }

    const { email, password } = validationResult.data
    console.log('[API Login] Datos validados - Email:', email)
    ...
    if (error) {
      ...
      return NextResponse.json(createErrorResponse(error.message), { status: 401 })
    }

    if (!data.session) {
      console.error('[API Login] No se pudo crear la sesión')
      return NextResponse.json(createErrorResponse('No se pudo crear la sesión'), { status: 401 })
    }
