...
export default function EditableCatalogForm({ id, initialData, onSaveSuccess }: Props) {
  ...
  async function handleSave() {
    try {
      ...
      console.log('Enviando datos a API:', { id: idStr, changes: { name, author, location } }) // Debug log
      ...
      console.log('Respuesta de API:', res.status, res.statusText) // Debug log
      
      const json = await res.json()
      console.log('JSON de respuesta:', json) // Debug log

      if (!res.ok || !json?.success) {
        const errorMsg = json?.error || json?.message || `Error ${res.status}: ${res.statusText}`
        throw new Error(errorMsg)
      }

      setMessage('Cambios guardados exitosamente')
      setConfirming(false) // Resetear estado de confirmación
      router.refresh()
      ...
      console.error('Error en handleSave:', e) // Debug log
      const msg = e instanceof Error ? e.message : 'Error guardando cambios'
      setMessage(`❌ ${msg}`)
      setConfirming(false) // Resetear estado de confirmación en caso de error
    } finally {
      setSaving(false)
    }
  }
