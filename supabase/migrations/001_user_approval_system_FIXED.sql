-- ============================================
-- SISTEMA DE APROBACIÓN Y SUSCRIPCIÓN DE USUARIOS (VERSIÓN CORREGIDA)
-- ============================================
-- Fecha: 2025-11-04
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE profiles ADD COLUMN user_status TEXT DEFAULT 'pending';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE profiles ADD CONSTRAINT profiles_user_status_check
    CHECK (user_status IN ('pending', 'approved_unpaid', 'active', 'suspended', 'rejected'));
  END IF;
EXCEPTION
...
-- Añadir constraint para payment_method
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.constraint_column_usage
    ...
    CHECK (payment_method IN ('bizum', 'kofi', 'manual', null));
  END IF;
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
-- Eliminar TODAS las políticas antiguas de items (sin importar el nombre)
